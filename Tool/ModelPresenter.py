...
class PresenterDialog(TranslatedDialog):
    def __init__(self, parent=None):
        super(PresenterDialog, self).__init__(parent)
    ...
    def toggle_play(self, checked):
        ...
        if checked:
            if not check_has_valid_models():
                ...
                QtWidgets.QMessageBox.information(self, translate_text("通知", lang), translate_text("モデルが存在しません。", lang))
                return
...
class Tool_ModelPresenter:
    ...
    def Activated(self):
        lang = get_language()
        if not check_has_valid_models():
            QtWidgets.QMessageBox.information(FreeCADGui.getMainWindow(), translate_text("通知", lang), translate_text("モデルを生成してください。", lang))
            return
