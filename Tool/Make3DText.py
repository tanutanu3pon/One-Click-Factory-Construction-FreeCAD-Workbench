...
class Tool_Make3DText:
    ...
    def Activated(self):
        ...
        if not text.strip() or not font_path or not os.path.exists(font_path):
            QtWidgets.QMessageBox.warning(None, "警告", "テキストまたはフォントが無効です。")
            return
        ...
        try:
            ...
            if not all_line_data:
                QtWidgets.QMessageBox.warning(None, "エラー", "3D文字を生成できませんでした。")
