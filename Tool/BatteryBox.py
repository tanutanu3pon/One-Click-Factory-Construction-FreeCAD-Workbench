...
from Core.Controller import TranslatedDialog  # 自動翻訳クラスを追加
...
class BatteryBoxDialog(TranslatedDialog):
...
class Tool_MakeBatteryBox:
    ...
    def Activated(self):
        ...
        b_type_idx, b_count, b_dir, wall_t, need_hole = d.get_values()
        self.create_battery_box(b_type_idx, b_count, b_dir, wall_t, need_hole)

    def create_battery_box(self, b_type_idx, b_count, b_dir, wall_t, need_hole):
        with Progress.ProgressManager() as bar:
            bar.start(title="電池BOX生成", initial_text="仕切り壁の計算中...")
            
            doc = FreeCAD.activeDocument() or FreeCAD.newDocument()
            
            keys = list(BATTERY_SPECS.keys())
            spec = BATTERY_SPECS[keys[b_type_idx]]
            slot_w = spec["d"] + spec["margin_d"]
            slot_l = spec["l"] + spec["margin_l"]
            radius = slot_w / 2.0
            
            divider_t = 1.8 if b_type_idx == 0 else 1.5
            ...
            wire_hole_radius = 1.25 if b_type_idx == 0 else 1.0
