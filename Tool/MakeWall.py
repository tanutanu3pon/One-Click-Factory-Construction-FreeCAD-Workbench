...
class ModeSelectDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        btn_box.button(QtWidgets.QDialogButtonBox.Ok).setText("次へ")
...
class UniformWallDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        btn_box = QtWidgets.QDialogButtonBox(QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel)
        btn_box.button(QtWidgets.QDialogButtonBox.Ok).setText("擁壁を生成")
        btn_box.accepted.connect(self.accept)
        btn_box.rejected.connect(self.reject)
...
class TwistWallDialog(QtWidgets.QDialog):
    ...
    def validate_and_accept(self):
        if self.spin_top_start.value() >= self.spin_base_start.value():
            QtWidgets.QMessageBox.warning(self, "入力エラー", "手前の「天端幅」は、「底面幅」よりも小さくしてください。")
            return
        if self.spin_top_end.value() >= self.spin_base_end.value():
            QtWidgets.QMessageBox.warning(self, "入力エラー", "奥方向の「天端幅」は、「底面幅」よりも小さくしてください。")
            return
        self.accept()
...
class Tool_MakeWall:
    ...
    def Activated(self):
        ...
        bar.start(title=f"重力式擁壁・{mode_title}", initial_text="既存の擁壁データをスキャン中...")
        ...
        try:
            ...
            QtWidgets.QMessageBox.information(None, "成功", f"第{next_idx}区間の擁壁が完成し、情報をプロパティに記録しました！")

        except Exception as e:
            FreeCAD.Console.PrintError(f"擁壁生成エラー: {str(e)}\n")
