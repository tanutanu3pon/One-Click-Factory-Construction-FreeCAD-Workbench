...
class CookieDialog(TranslatedDialog):
    ...
    def select_image(self):
        ...
        path, _ = QtWidgets.QFileDialog.getOpenFileName(self, title_text, "", file_filter)
        if path:
...
class Tool_MakeCookie:
    ...
    def Activated(self):
        ...
        with Progress.ProgressManager() as bar:
            bar.start(title=translate_text("クッキー型枠・立体成形", lang), initial_text=translate_text("画像をSVG風に高精度トレース中...", lang))
            ...
            try:
                ...
                bar.update(30, translate_text("AI画像処理で厚みを計算中...", lang))
                ...
                bar.update(50, translate_text("曲線の最適化中...", lang))
                ...
                bar.update(70, translate_text("立体化のための面を構築中...", lang))
                ...
                msg_extrude = f"Extruding {vals['height']}mm..." if lang == "English" else f"{vals['height']}mm 押し出して成形中..."
                bar.update(80, msg_extrude)
                ...
                bar.update(90, translate_text("中身をくり抜いて空洞にしています...", lang))
                ...
                if lang == "English":
                    title_succ = "Success"
                    msg_succ = "Successfully created hollow 3D cookie cutter!"
                else:
                    title_succ = "成功"
                    msg_succ = "中身を完璧にくり抜いた、本物のクッキー型枠が完成しました！"
                
                QtWidgets.QMessageBox.information(None, title_succ, msg_succ)

            except Exception as e:
                FreeCAD.Console.PrintError(f"Cookie cutter error: {str(e)}\n")
                err_title = "Error" if lang == "English" else "エラー"
                err_msg = f"An error occurred during 3D modeling:\n{str(e)}" if lang == "English" else f"立体化中にエラーが発生しました:\n{str(e)}"
