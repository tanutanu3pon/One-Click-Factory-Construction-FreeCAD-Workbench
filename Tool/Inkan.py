...
class Tool_Inkan:
    ...
    def Activated(self):
        ...
        # 3. 高さ（長さ）の入力（シンプルな印鑑のときだけ個別に聞く）
        if "シンプルな" in selected_type:
        ...
        # 生成・彫刻関数の呼び出し
        self.create_and_carve_inkan(selected_type, size, length, fillet_top, text_str, text_depth)

    def create_and_carve_inkan(self, selected_type, size, length, fillet_top, text_str, text_depth):
        # ==========================================
        ...
        # 1. 印鑑の土台（ソリッド）を作成
        # ==========================================
        bar.update(15, f"1/3: {selected_type} の土台ソリッドを構築中...")
        
        r_base = size / 2.0
        h_base = 8.0 if "スタンプ" in selected_type else length

        if "丸印 (シンプルな円柱)" == selected_type:
            base_shape = Part.makeCylinder(r_base, length)
            label = f"Inkan_Maru_{text_str}"
        elif "角印 (シンプルな四角柱)" == selected_type:
            half_s = size / 2.0
        ...
        # 天面のフィレット処理（シンプルな印鑑のみ）
        if fillet_top and "シンプルな" in selected_type:
            edges_to_fillet = []
        ...
        try:
            ...
            bar.close() # エラーの時も確実に進捗バーの凍結を解除する
