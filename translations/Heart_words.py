...
class Tool_Heart:
    ...
    def Activated(self):
        ...
        item_text, ok3 = QtWidgets.QInputDialog.getItem(None, "形状選択", "ハートのタイプ:", items, 1, False)
        if not ok3: return
        
        # ★英語化対策：選ばれた文字列のインデックス番号からふっくらタイプを判定（インデックス1が「ふっくら」）
        item_idx = items.index(item_text) if item_text in items else 1
        is_puffy = (item_idx == 1)

        # 4. 紐通し用の穴を設けるかどうかの選択窓
        hole_items = ["穴を設ける", "穴を設けない"]
        hole_choice_text, ok4 = QtWidgets.QInputDialog.getItem(None, "ハート設計", "紐通し穴の設定:", hole_items, 0, False)
        if not ok4: return
        
        # ★英語化対策：インデックス番号から穴の有無を判定（インデックス0が「穴を設ける」）
        hole_idx = hole_items.index(hole_choice_text) if hole_choice_text in hole_items else 0
        has_hole = (hole_idx == 0)
        r_hole = 0.8 # デフォルト値
